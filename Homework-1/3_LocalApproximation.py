...
With the following approximation functions :math:`f_h(x)`:
1. a Taylor series expansion around :math:`x=0.5`,
2. a Fourier series,
3. a global polynomial and,
4. a local approximations within finite cells:
    a. using linear approximators and,
    b. using quadratic approximators.

Test these approximatiors using two errors:
...
def Taylor_series(N, x):
    ...
    -------
    array_like
        Taylor_series at points :math:`x`, that is :maht:`f_h(x)`.
    """
    # Hardcode coeficients.
...
def Fourier_coefs(fun, N, x):
    ...
    # Perform fast foureier anaylysis.
...
def Polynomial(fun, N, x):
    ...
    # Obtain locations where we fit the polinomial and fit.
